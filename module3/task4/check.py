...
def check_decrypt_answers(answers):
    ...
    for i in range(len(TESTDATA)):
        ...
        result += 1 if TESTDATA[i][0] == answers[i] else 0
    return result
...
if not failed:
    with open(encrypt_filename, 'r') as encrypt_file, open(decrypt_filename, 'r') as decrypt_file:
        right_answers = min(check_encrypt_answers(get_encrypt_student_answers(encrypt_file.read())),\
                            check_decrypt_answers(get_decrypt_student_answers(decrypt_file.read())))
